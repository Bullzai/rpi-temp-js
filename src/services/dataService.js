...
function formatDate(isoString) {
  ...
  const [_, month, day, year, hour, minute] = formatted.match(
    /(\d{2})\/(\d{2})\/(\d{4}), (\d{2}):(\d{2})/
  );
...
export function updateReadingsFile(temperatureData) {
  ...
  const newReading = {
    ...
    sensor1_temperature: temperatureData.sensor1_temperature,
    sensor2_temperature: temperatureData.sensor2_temperature,
  };
